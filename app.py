...
def build_execution_plan(diagnosis: dict, weather: dict) -> str:
    ...
    # Custom plan for Yield/Soil modes which don't have severity
...
def analyze():
    ...
    if lat_f and lon_f:
        ...
        weather = {"spray_status": "Unknown", "status_reason": "No GPS"}
    ...
    response_data = {
        "success": True,
        "scan_id": record.id,
        "diagnosis": diagnosis,  # Send full dict to support new fields (ROI, Yield, etc.)
        "weather": weather,
        "execution_plan": execution_plan,
...
def api_scans():
    records = db.session.query(ScanRecord).order_by(ScanRecord.timestamp.desc()).limit(500).all()
